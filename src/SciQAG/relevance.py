...
def input_format(data):
    ...
    # questions = qas[0][0]
    prompt = f"Given a scientific paper and questions generated from it, evaluate the relevance of the question to the paper and return a score ranging from 1–3 and give reasons as to why this score was assigned. The output must be a list of dictionaries corresponding to each question, with the fields ‘score’ and ‘reasons.’ If the question does not pertain to the paper, assign a score of 1. Only output a number.\nPaper: {paper} \nQuestions: {[{questions}]} \nOutput:"
    return prompt
...
file_path = '/root/SciQAG/relevance.json'
...
res = read('/root/SciQAG/relevance.json')
